...
abstract class MainDialog
{
    ...
    internal virtual async Task<bool> ShowAsync(XamlElement element) => await PromptAsync(element) != ContentDialogResult.None;

    internal async Task<ContentDialogResult> PromptAsync(XamlElement element)
    {
        await s_semaphore.WaitAsync();
        try
        {
            _dialog.XamlRoot = ((UIElement)element).XamlRoot;
...
{
    internal override async Task<bool> ShowAsync(XamlElement element)
    {
